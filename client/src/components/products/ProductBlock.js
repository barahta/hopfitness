...
function ProductBlock() {
    ...
    const getPacks = async () => {
        try{
            const {data} = await NewsService.getAllPacks({capter: 'hopefitness'})
            if(data){
                console.log(data)
                data.map((card, index)=>{

                        card.imgSrc=""
                        card.description=card.publicdesc

                        card.content= card.desc
                        card.title='КЛУБНАЯ КАРТА - '+card.name
                    if(index+1 === 5){
                        card.super= 'super'
                    }else{
                        card.super= ''
    ...
    return (
        <div className={style.main}>
            ...
            <div className={style.container}>
                {(products && products.length>0)&&products.map((product, indexProd) => (
                    <div
                        key={indexProd}
                        className={`${style.productCard} ${visible.includes(indexProd.toString()) ? style.visible : ''} ${product.super}`}
                        data-index={indexProd}
                        onClick={()=>{setData(product); setActivemodal(true)}}
                    >
                        <div className={`${style.cardContainer} ${product.styles.cardContainer}`}>
                            <div className={`${style.title} ${product.styles.title}`}>{product.name}</div>
                            <div className={`${style.overlay} ${product.styles.overlay}`}>
                                <div className={`${style.priceTag} ${product.styles.priceTag}`}>{product.price}</div>
                                <div className={`${style.description} ${product.styles.description}`} style={(product.super === 'super')?{color: '#454545'}:{}}>{product.description}</div>
                            </div>
                        </div>
                    </div>
                ))}
